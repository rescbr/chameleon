...
/*#if 0
    ...
    uint32_t offset;
    for (offset = 0; (offset < 0xFFFFFF00) && memcmp((uint8_t *)binary + offset, (uint8_t *)gDarwinBuildVerStr, 21); offset++) {
    }
    if (offset < 0xFFFFFF00) gDarwinBuildVerStr = (char *)binary + offset;
    else gDarwinBuildVerStr = "";
