...
static int countdown( const char * msg, int row, int timeout )
{
    ...
    if ((gMacOSVersion[0] == '1') && (gMacOSVersion[1] == '0') && (gMacOSVersion[2] == '.') && (gMacOSVersion[3] >= '7'))
    {
		...
		if( bootArgsSnow->Video.v_display == VGA_TEXT_MODE )        
		{
			moveCursor( 0, row );
			printf(msg);
    ...
    for ( time = time18(), timeout++; timeout > 0; )
    {
        ...
        if ((gMacOSVersion[0] == '1') && (gMacOSVersion[1] == '0') && (gMacOSVersion[2] == '.') && (gMacOSVersion[3] >= '7'))
        {
			...
			if( bootArgsSnow->Video.v_display == GRAPHICS_MODE )
			{
				drawProgressBar( gui.screen.pixmap, 100, gui.progressbar.pos , ( multi * 100 / multi_buff ) );
				gui.redraw = true;
				updateVRAM();
...
processBootOptions()
{
    ...
    //rooting via /chosen/ now, no need anymore
    //    if (processBootArgument(kBootUUIDKey, cp, configKernelFlags, bootInfo->config, &argP, &cntRemaining, 0)) {
    if (!processBootArgument(kNoUseBootUUIDKey, cp, configKernelFlags, bootInfo->config, &argP, &cntRemaining, 0)) {
