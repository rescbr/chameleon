...
#define VIDEOSNOW(x) (bootArgsSnow->Video.v_ ## x)
					...
					unsigned short refreshRate )
{
    ...
    do {
        ...
        bootArgsSnow->Video.v_display  = GRAPHICS_MODE;
        bootArgsSnow->Video.v_width    = minfo.XResolution;
        bootArgsSnow->Video.v_height   = minfo.YResolution;
        bootArgsSnow->Video.v_depth    = minfo.BitsPerPixel;
        bootArgsSnow->Video.v_rowBytes = minfo.BytesPerScanline;
        bootArgsSnow->Video.v_baseAddr = VBEMakeUInt32(minfo.PhysBasePtr);
    }
    while ( 0 );
				...
				uint8_t *data)
{
    ...
    if ((gMacOSVersion[0] == '1') && (gMacOSVersion[1] == '0') && (gMacOSVersion[2] == '.') && (gMacOSVersion[3] >= '7'))
    {
        vram = (uint8_t *) VIDEO(baseAddr) + VIDEO(rowBytes) * y + 4 * x;
    	drawWidth = MIN(width, VIDEO(width) - x);
    	height = MIN(height, VIDEO(height) - y);
    } else {
        vram = (uint8_t *) VIDEOSNOW(baseAddr) + VIDEOSNOW(rowBytes) * y + 4 * x;        
        drawWidth = MIN(width, VIDEOSNOW(width) - x);
        height = MIN(height, VIDEOSNOW(height) - y);
    }
    while (height--) {
        if ((gMacOSVersion[0] == '1') && (gMacOSVersion[1] == '0') && (gMacOSVersion[2] == '.') && (gMacOSVersion[3] >= '7'))
        {
...
void drawCheckerBoard()
{
    ...
    if ((gMacOSVersion[0] == '1') && (gMacOSVersion[1] == '0') && (gMacOSVersion[2] == '.') && (gMacOSVersion[3] >= '7'))
        vram = (uint32_t *) VIDEO(baseAddr);
    else
        vram = (uint32_t *) VIDEOSNOW(baseAddr);
    ...
    if ((gMacOSVersion[0] == '1') && (gMacOSVersion[1] == '0') && (gMacOSVersion[2] == '.') && (gMacOSVersion[3] >= '7'))
    {
		...
		for (y = 0; y < VIDEOSNOW(height); y++, vram += VIDEOSNOW(width)) {
			for (x = 0; x < VIDEOSNOW(width); x++) {
				color = 204 + 51 * (((x / 8) % 2) == ((y / 8) % 2));
				vram[x] = (color << 16) | (color << 8) | color;
			}
		}
						...
						unsigned char  colorIndex )
{
    ...
    if ((gMacOSVersion[0] == '1') && (gMacOSVersion[1] == '0') && (gMacOSVersion[2] == '.') && (gMacOSVersion[3] >= '7'))
        color = lookUpCLUTIndex( colorIndex, VIDEO(depth) );
    else
        color = lookUpCLUTIndex( colorIndex, VIDEOSNOW(depth) );
    ...
    if ((gMacOSVersion[0] == '1') && (gMacOSVersion[1] == '0') && (gMacOSVersion[2] == '.') && (gMacOSVersion[3] >= '7'))
    {
		...
		pixelBytes = VIDEOSNOW(depth) / 8;
		vram       = (char *) VIDEOSNOW(baseAddr) +
		VIDEO(rowBytes) * y + pixelBytes * x;
		
		width = MIN(width, VIDEOSNOW(width) - x);
		height = MIN(height, VIDEOSNOW(height) - y);
	}
					   ...
					   unsigned char * data )
{
    ...
    if ((gMacOSVersion[0] == '1') && (gMacOSVersion[1] == '0') && (gMacOSVersion[2] == '.') && (gMacOSVersion[3] >= '7'))
    {
		...
		long   pixelBytes = VIDEOSNOW(depth) / 8;
		unsigned char * vram   = (unsigned char *) VIDEOSNOW(baseAddr) +
		VIDEOSNOW(rowBytes) * y + pixelBytes * x;
...
void drawPreview(void *src, uint8_t * saveunder)
{
	...
	if (src && (uncomp=DecompressData(src, &origwidth, &origheight, &origbpx)))
	{
		...
		if ((gMacOSVersion[0] == '1') && (gMacOSVersion[1] == '0') && (gMacOSVersion[2] == '.') && (gMacOSVersion[3] >= '7'))
		{
			...
			screen = (uint8_t *) VIDEOSNOW (baseAddr);
			rowBytes = VIDEOSNOW (rowBytes);
			loadImageScale (uncomp, origwidth, origheight, origbpx, screen, VIDEOSNOW(width), VIDEOSNOW(height), VIDEOSNOW(depth), VIDEOSNOW (rowBytes));
		}
	}
	else
	{
        ...
        if ((gMacOSVersion[0] == '1') && (gMacOSVersion[1] == '0') && (gMacOSVersion[2] == '.') && (gMacOSVersion[3] >= '7'))
        {
			...
			rowBytes = VIDEOSNOW (rowBytes);
			// Set the screen to 75% grey.
			drawColorRectangle(0, 0, VIDEOSNOW(width), VIDEOSNOW(height), 0x01 /* color index */);
		}
	}
    ...
    if ((gMacOSVersion[0] == '1') && (gMacOSVersion[1] == '0') && (gMacOSVersion[2] == '.') && (gMacOSVersion[3] >= '7'))
    {
		...
		screen += ((VIDEOSNOW (width) 
					- kIOHibernateProgressCount * (kIOHibernateProgressWidth + kIOHibernateProgressSpacing)) << (pixelShift - 1))
		+ (VIDEOSNOW (height) - kIOHibernateProgressOriginY - kIOHibernateProgressHeight) * rowBytes;        
	}
				...
				unsigned char  bitsPerPixel )
{
    ...
    bootArgsSnow->Video.v_display  = VGA_TEXT_MODE;
    bootArgsSnow->Video.v_baseAddr = 0xb8000;
    bootArgsSnow->Video.v_width    = minfo.XResolution;
    bootArgsSnow->Video.v_height   = minfo.YResolution;
    bootArgsSnow->Video.v_depth    = 8;
    bootArgsSnow->Video.v_rowBytes = 0x8000;
    ...
    if ( mode == GRAPHICS_MODE )
    {
  		if ( (err=initGraphicsMode ()) == errSuccess ) {
			if (gVerboseMode) {
				...
				bootArgsSnow->Video.v_display = FB_TEXT_MODE;            
			} else {
				bootArgs->Video.v_display = GRAPHICS_MODE;
				bootArgsSnow->Video.v_display = GRAPHICS_MODE;
			}
...
int getVideoMode(void)
{
    ...
    return bootArgsSnow->Video.v_display;
}
