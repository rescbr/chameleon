			...
			while(gMI->mi_mods_count--)
			{
				if(mod->mm_string)
				{
					...
					if(strcmp(&mod->mm_string[strlen(mod->mm_string) - sizeof("dylib")], ".dylib") == 0)
					{
						...
						verbose("Loading multiboot module '%s'...\n", name);
						...
						if(module_start && (module_start != (void*)0xFFFFFFFF))
						{
							...
							verbose("Multiboot module '%s' started.\n", name);
						}
					}
				}
			}
...
void load_all_modules()
{
	...
	while (readdir(moduleDir, (const char**)&name, &flags, &time) >= 0) {
		...
		{
			//verbose("[load_all_modules] Ignoring file: '%s'\n", name);
		}
...
int load_module(char* module)
{
	...
	if(fh < 0)
	{
		verbose("[WARNING!] Unable to locate module '%s'. Not loaded.\n", modString);
		return 0;
	}
...
)
{
	...
	{
		error("[ERROR!] Invalid mach magic: 0x%08X.\n", ((struct mach_header*)binary)->magic);
		return NULL;
	}
	...
	while(cmd < ((struct mach_header*)binary)->ncmds)
	{
		...
		switch ((loadCommand->cmd & 0x7FFFFFFF))
		{
			...
			default:
				verbose("[WARNING!] Unhandled loadcommand: 0x%08X.\n", loadCommand->cmd & 0x7FFFFFFF);
				break;
...
{
	verbose("Attempting to execute hook '%s'...\n", name);
	...
	if(hook)
	{
		...
		verbose("Hook '%s' executed.\n", name);
		return 1;
	}
	else
	{
		// Callback for this hook doesn't exist;
		verbose("No callbacks for hook '%s'.\n", name);
		return 0;
	}
}
...
{
	verbose("Adding callback for '%s' hook... ", name);
