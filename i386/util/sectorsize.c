...
//  sectorsize
//
//  Created by Micky1979 on 15/11/14.
...
int main(int argc, const char * argv[]) {
    ...
    if (getuid()) {
        show_usage();
        printf("%s", "Error: You must run this command as root user!\n-------------------------------------------------------------------------------\n");
        exit(77);
    }
    ...
    {
        show_usage();
        printf("Error: a /dev/diskX argument was expected.\n-------------------------------------------------------------------------------\n");
        exit(64);
    }

    uint32_t size = 0;
    uint32_t diskdevice = open(argv[1],O_RDONLY);
