...
bool validate_rom(struct pci_rom_bios_t *rom_header, pci_dt_t *pci_dev)
{
	...
	rom_pci_header = (struct pci_rom_pci_header_t *)((uint8_t *)rom_header + rom_header->pci_header);
	
	if (rom_pci_header->signature != 0x52494350)
		return false;
	
	if (rom_pci_header->vendor != pci_dev->vendor_id || rom_pci_header->device != pci_dev->device_id)
		return false;
...
bool load_vbios_file(const char *key, uint16_t vendor_id, uint16_t device_id, uint32_t subsys_id)
{
	...
	if (!validate_rom((struct pci_rom_bios_t *)card->rom, card->pci_dev))
	{
		card->rom_size = 0;
		card->rom = 0;
		return false;
	...
	card->rom_size = ((struct pci_rom_bios_t *)card->rom)->size * 512;
...
bool read_vbios(bool from_pci)
{
	struct pci_rom_bios_t *rom_addr;
	
	if (from_pci)
	{
		rom_addr = (struct pci_rom_bios_t *)(pci_config_read32(card->pci_dev->dev.addr, PCI_ROM_ADDRESS) & ~0x7ff);
		verbose(" @0x%x", rom_addr);
	}
	else
		rom_addr = (struct pci_rom_bios_t *)0xc0000;
...
static bool init_card(pci_dt_t *pci_dev)
{
	...
	for (i = 0; radeon_cards[i].device_id ; i++)
	{
		if (radeon_cards[i].device_id == pci_dev->device_id)
		{
			...
			else if (radeon_cards[i].subsys_id == pci_dev->subsys_id.subsys_id)
				break;
		}
	}
