...
struct acpi_2_fadt *
patch_fadt(struct acpi_2_fadt *fadt, struct acpi_2_facs *new_facs, struct acpi_2_dsdt *new_dsdt, bool UpdateFADT)
{		
	...
	bool aspm = false;
	if (getBoolForKey(kEnableASPM, &aspm, &bootInfo->bootConfig) && aspm) {
		fadt_mod->Boot_Flags = 0x0000; 
	}
	

	// Patch FACS Address
	if (new_facs) {
		fadt_mod->FIRMWARE_CTRL=(uint32_t)new_facs;	
		if ((uint32_t)(&(fadt_mod->X_FIRMWARE_CTRL))-(uint32_t)fadt_mod+8<=fadt_mod->Length)
			fadt_mod->X_FIRMWARE_CTRL=(uint32_t)new_facs;
		Platform->hardware_signature = 	new_facs->hardware_signature;
	} else {
...
static uint32_t const MACH_SIG_MP		= 0x00000200;
static uint32_t const MACH_SIG_ZERO		= 0x00000000;

struct acpi_2_facs* generate_facs(bool updatefacs)
{
	struct acpi_2_facs *facs_mod=(struct acpi_2_facs *)AllocateKernelMemory(sizeof(struct acpi_2_facs));
	bzero(facs_mod, sizeof(struct acpi_2_facs));
	
	memcpy(facs_mod->Signature, "FACS", 4);
	facs_mod->Length = sizeof(struct acpi_2_facs);
	
	if (!gPlatformName)
		local_readSMBIOS(1);
	
	//TODO : populate
	if (strstr("MacPro", gPlatformName) == 0) {
		facs_mod->hardware_signature = MACH_SIG_MP;
	}
	else {
		facs_mod->hardware_signature = MACH_SIG_ZERO;
	}
	facs_mod->firmware_waking_vector = 0;
	facs_mod->global_lock = 0;
	facs_mod->flags = 0;
	facs_mod->x_firmware_waking_vector = 0;
	
	facs_mod->version = updatefacs; /* ACPI 1.0: 0, ACPI 2.0/3.0: 1, ACPI 4.0: 2 */
	return facs_mod;
}
...
{	
	int version;
	void *new_dsdt=NULL, *new_hpet=NULL, *new_sbst=NULL, *new_ecdt=NULL, *new_asft=NULL, *new_dmar=NULL, *new_apic=NULL, *new_mcfg=NULL, *new_facs=NULL, *new_ssdts[14];
	
	struct acpi_2_ssdt *new_ssdt[17]; // 15 + 2 additional tables for pss & cst 
	struct acpi_2_fadt *fadt; // will be used in CST generator
	
	bool oem_facs=false, oem_dsdt=false, oem_ssdt=false, oem_hpet=false, oem_sbst=false, oem_ecdt=false, oem_asft=false, oem_dmar=false, oem_apic=false, oem_mcfg=false;
	...
	bool gen_facs=false, gen_csta=false, gen_psta=false, speed_step=false;
	...
	
	{
		...
		oem_facs=getBoolForKey(kOEMFACS, &tmpval, &bootInfo->bootConfig)&&tmpval;
		...
		gen_facs=getBoolForKey(kGenerateFACS, &tmpval, &bootInfo->bootConfig)&&tmpval;
	...
	if (!oem_facs)
		new_facs=loadACPITable(kFACS);
	
	if (!new_facs && gen_facs) new_facs = generate_facs(update_acpi);
	...
	for (version=0; version<2; version++) {
		...
		// Find original rsdp
		struct acpi_2_rsdp *rsdp=(struct acpi_2_rsdp *)(version?((struct acpi_2_rsdp *)(uint32_t)acpi20_p):((struct acpi_2_rsdp *)(uint32_t)acpi10_p));
		...
		{			
			...
			for (i=0;i<rsdt_entries_num;i++)
			{
				...
				{					
					...
					struct acpi_2_fadt *fadt_mod = patch_fadt(fadt, new_facs, new_dsdt, update_acpi);
