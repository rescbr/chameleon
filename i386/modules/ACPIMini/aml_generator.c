...
struct aml_chunk* aml_add_package_with_value(struct aml_chunk* parent, uint64_t *lst, int num)
{
	struct aml_chunk* node = aml_create_node(parent);
	...
	if (node)
	{
		...
		node->Length = 1;
		node->Buffer = malloc(node->Length);
		aml_add_multiple_value(node,lst,num);
...
{	
	...
	{			
		if (val > 0xffffffff)
			aml_add_qword(parent,val);
		else if (val > 0xffff)
			aml_add_dword(parent,val);
		else if (val > 0xff)
			aml_add_word(parent,val);
		else 
			aml_add_byte(parent, val);					
	}
...
void aml_add_ressource_t_fxhw(struct aml_chunk* parent, uint64_t *lst)
{
	if (parent) {
		...
		char *buffer = resource_template;
		unsigned int offset = aml_write_byte(lst[0], buffer, 8);
		offset = aml_write_byte(lst[1], buffer, offset);
		offset = aml_write_byte(lst[2], buffer, offset);
		aml_write_qword(lst[3], buffer, offset);		
		aml_add_buffer(parent, buffer, sizeof(resource_template));
	}
}
