    ...
    png_bytep output, png_size_t output_size)
{
   ...
   while (1)
   {
      ...
      ret = inflate(&png_ptr->zstream, Z_NO_FLUSH);
       ...
       execut_hook("inflate", &png_ptr->zstream, &flush, &ret, NULL, NULL, NULL );
#endif
...
#else
       int resetret;
       execut_hook("inflateReset", &png_ptr->zstream  &resetret, NULL, NULL, NULL, NULL );
#endif
      if (ret == Z_STREAM_END)
...
png_read_finish_row(png_structp png_ptr)
{
   ...
   if (png_ptr->idat_size || png_ptr->zstream.avail_in)
    ...
    inflateReset(&png_ptr->zstream);
#else
    execut_hook("inflateReset", &png_ptr->zstream  NULL, NULL, NULL, NULL, NULL );
#endif
   png_ptr->mode |= PNG_AFTER_IDAT;
}
