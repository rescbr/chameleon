...
static EFI_CHAR16 const FIRMWARE_VENDOR[] = {'C','h','a','m','e','l','e','o','n','_','2','.','5','M', 0};
...
//	memcpy((void*)&uuid, p, UUID_LEN);
	//LE byte order while BE in SMBIOS
	u = (SMBByte *)&uuid;
	u[0] = p[3]; u[1] = p[2]; u[2] = p[1]; u[3] = p[0];
	u[4] = p[5]; u[5] = p[4]; u[6] = p[7]; u[7] = p[6];
	for (i=8; i<16; i++) {
		u[i] = p[i];
	}
	return &uuid;
...
static void setupSmbiosConfigFile(const char *filename)
{
	...
	getSmbios(SMBIOS_ORIGINAL);  //Slice - first encounter
	scan_mem(); 
	smbios_p = (EFI_PTR32)getSmbios(SMBIOS_PATCHED);	// process smbios asap
//Slice - no more getSMBIOS needed
...
//	if ((Platform->CPU.Features & CPU_FEATURE_MOBILE)) {
		...
		(p->currentClock < 10000)))
	{
		...
		p->maximumClock = p->currentClock;
		Platform->CPU.TSCFrequency = p->currentClock * MEGA;
	}
}
...
{
//	smbios_p = (EFI_PTR32)getSmbios(SMBIOS_PATCHED);  //cached
