...
/* Apple EFI vendor + revision values. */
static EFI_CHAR16 const FIRMWARE_VENDOR[] = {'A','p','p','l','e'};
static EFI_UINT32 const FIRMWARE_REVISION = 0x0001000a;

/* Default platform system_id (fix by IntVar) */
static EFI_CHAR8 const SYSTEM_ID[] = {0x80,0xc0,0x0b,0x65,0xc8,0x09,0xf4,0xbe,0x6a,0xee,0x0e,0xed,0x02,0xec,0x31,0x88};//MacBook6,1 value
...
void setupEfiTables32(void)
{
    ...
    bootArgsSnow->efiSystemTable = (uint32_t)efiSystemTable;
    bootArgsSnow->efiMode = kBootArgsEfiMode32;
...
void setupEfiTables64(void)
{
    ...
    bootArgsSnow->efiSystemTable = (uint32_t)efiSystemTable;
    bootArgsSnow->efiMode = kBootArgsEfiMode64;
...
void setupBoardId()
{
    ...
    if (boardid)
        DT__AddProperty(node, "board-id", strlen(boardid)+1, (EFI_CHAR16*)boardid);
