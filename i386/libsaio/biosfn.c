...
APMPresent(void)
{
    ...
    if ((bb.flags.cf == 0) &&
        ...
        bootArgsSnow->apmConfig.major_vers = bb.eax.r.h;
        bootArgsSnow->apmConfig.minor_vers = bb.eax.r.l;
        bootArgsSnow->apmConfig.flags.data = bb.ecx.rr;
...
APMConnect32(void)
{
    ...
    if (bb.flags.cf == 0) {
        ...
        bootArgsSnow->apmConfig.cs32_base = (bb.eax.rr) << 4;
        bootArgsSnow->apmConfig.entry_offset = bb.ebx.rx;
        bootArgsSnow->apmConfig.cs16_base = (bb.ecx.rr) << 4;
        bootArgsSnow->apmConfig.ds_base = (bb.edx.rr) << 4;
        if (bootArgsSnow->apmConfig.major_vers >= 1 &&
            bootArgsSnow->apmConfig.minor_vers >= 1) {
            bootArgsSnow->apmConfig.cs_length = bb.esi.rr;
            bootArgsSnow->apmConfig.ds_length = bb.edi.rr;
        } else {
            bootArgsSnow->apmConfig.cs_length = 
            bootArgsSnow->apmConfig.ds_length = 64 * 1024;
        }
        bootArgsSnow->apmConfig.connected = 1;
