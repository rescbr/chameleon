...
void initKernBootStruct( void )
{
    ...
    if ( !init_done )
    {
        ...
        if (bootArgsSnow == 0 || bootArgs == 0 || bootInfo == 0)
            stop("Couldn't allocate boot info\n");
		
        bzero(bootArgs, sizeof(boot_args));
        bzero(bootArgsSnow, sizeof(boot_args_snow));
		...
		bootArgsSnow->Version = kBootArgsVersion_Snow;
		bootArgsSnow->Revision = kBootArgsRevision1_5;
...
reserveKernBootStruct(void)
{
    if ((gMacOSVersion[0] == '1') && (gMacOSVersion[1] == '0') && (gMacOSVersion[2] == '.') && (gMacOSVersion[3] >= '7'))
    {
		...
		void *oldAddr = bootArgsSnow;
		bootArgsSnow = (boot_args_snow *)AllocateKernelMemory(sizeof(boot_args_snow));
		bcopy(oldAddr, bootArgsSnow, sizeof(boot_args_snow));
	}
}
...
finalizeBootStruct(void)
{
    ...
    bootArgsSnow->MemoryMap = (uint32_t)memoryMap;
    bootArgsSnow->MemoryMapSize = sizeof(EfiMemoryRange) * memoryMapCount;
    bootArgsSnow->MemoryMapDescriptorSize = sizeof(EfiMemoryRange);
    bootArgsSnow->MemoryMapDescriptorVersion = 0;
    ...
    bootArgsSnow->deviceTreeP = (uint32_t)addr;
    bootArgsSnow->deviceTreeLength = size;
}
