...
bool setup_gma_devprop(pci_dt_t *gma_dev)
{
	...
	if (model == "Mobile GMA950") {
		devprop_add_value(device, "AAPL,HasPanel", reg_TRUE, 4);
		devprop_add_value(device, "built-in", (uint8_t)0x00, 1);
		devprop_add_value(device, "AAPL,HasPanel", (uint8_t)0x00, 1); // needed for some of the cards
	} else if (model == "Desktop GMA950") {
		devprop_add_value(device, "built-in", (uint8_t)0x01, 1);
	} else if (model == "GMAX3100") {
