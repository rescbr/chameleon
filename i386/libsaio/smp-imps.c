...
#define PHYS_TO_VIRTUAL(x)	ptov(x)  /* convert physical address "x" to virtual */
#define VIRTUAL_TO_PHYS(x)	vtop(x)  /* convert virtual address "x" to physical */
...
static int  imps_scan(unsigned start, unsigned length);
static void imps_read_bios(imps_fps *fps_ptr);
static int  imps_bad_bios(imps_fps *fps_ptr);
static void imps_read_config_table(unsigned start, int count);
static void add_ioapic(imps_ioapic *ioapic);
static void add_bus(imps_bus *bus);
...
add_processor(imps_processor *proc)
{
	...
	msglog("  Processor [APIC id %d ver %d]:  ",
	...
	if (proc->flags & (IMPS_CPUFLAG_BOOT)) {
		msglog("#0  BootStrap Processor (BSP)\n");
		return;
	}
#if BOOT_CPU
	if (boot_cpu(proc)) {
...
imps_bad_bios(imps_fps *fps_ptr)
{
	...
	if (fps_ptr->feature_info[0] > IMPS_FPS_DEFAULT_MAX) {
		DBG("    Invalid MP System Configuration type %d\n",
	...
	if (fps_ptr->cth_ptr) {
		...
		if (local_cth_ptr->extended_length) {
			...
			if (sum) {
				DBG("    Bad Extended MP Config Table checksum 0x%x\n", sum);
				return 1;
			}
		}
	} else if (!fps_ptr->feature_info[0]) {
		DBG("    Missing configuration information\n");
		return 1;
	}
...
imps_read_bios(imps_fps *fps_ptr)
{
	...
	if (imps_bad_bios(fps_ptr)) {
		DBG("    Disabling MPS support\n");
		return;
	}
	
	verbose("Intel MultiProcessor Spec 1.%d BIOS support detected\n",
		   fps_ptr->spec_rev);
	...
	msglog("    APIC config: \"%s mode\"    Local APIC address: 0x%x\n",
		      str_ptr, imps_lapic_addr);
	//msr_t msr32 = rdmsr(0x1b); 
	if (imps_lapic_addr != (rdmsr(0x1b).lo & 0xFFFFF000)) {
		DBG("Inconsistent Local APIC address, Disabling SMP support\n");
		return;
	}
...
{
	DBG("Scanning from 0x%x for %d bytes\n",
			  start, length);

	while (length > 0) {
		gfps_ptr = (imps_fps *) PHYS_TO_VIRTUAL(start);

		if (gfps_ptr->sig == IMPS_FPS_SIGNATURE
		 && gfps_ptr->length == 1
		 && (gfps_ptr->spec_rev == 1 || gfps_ptr->spec_rev == 4)
		 && !get_checksum(start, 16)) {
			DBG("Found MP Floating Structure Pointer at %x\n", start);
			imps_read_bios(gfps_ptr);
...
imps_force(int ncpus)
{
	...
	DBG("Intel MultiProcessor \"Force\" Support\n");

	imps_lapic_addr = (rdmsr(0x1b).lo & 0xFFFFF000);
	imps_lapic_addr = PHYS_TO_VIRTUAL(imps_lapic_addr);
