...
vBiosMap * openVbios(chipsetType forcedChipset)
{
	...
	else if (forcedChipset != CT_UNKNOWN)
	{
...
//	map->hasSwitched = false;
	...
	if ( (map->chipset == CT_UNKNOWN) || ((map->bios != BT_ATI_1) && (map->bios != BT_NVDA) && (map->bios != BT_1)) )
	{
...
//	extern void clearBootArgs(void); Azi: declared on options.h
	...
	if ((gAutoResolution == true) && useGUI && map ) //Azi: should this work on TEXTMODE ???
	{
		// get the new Graphics Mode key
		processBootOptions(); //Azi: use processBootArgument instead?
//		processBootArgument(kGraphicsModeKey, cp, configKernelFlags,
//							bootInfo->config, &argP, &cntRemaining, gRootDevice)
...
void finishAutoRes()
{
	...
	else if (map) // assume (gAutoResolution == true)
	{
