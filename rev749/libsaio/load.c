...
long DecodeMachO(void *binary, entry_t *rentry, char **raddr, int *rsize)
{
  ...
  verbose("magic:      %x\n", (unsigned)mH->magic);
  verbose("cputype:    %x\n", (unsigned)mH->cputype);
  verbose("cpusubtype: %x\n", (unsigned)mH->cpusubtype);
  verbose("filetype:   %x\n", (unsigned)mH->filetype);
  verbose("ncmds:      %x\n", (unsigned)mH->ncmds);
  verbose("sizeofcmds: %x\n", (unsigned)mH->sizeofcmds);
  verbose("flags:      %x\n", (unsigned)mH->flags);
  getc();
#endif
...
#if DEBUG
  verbose("segname: %s, vmaddr: %x, vmsize: %x, fileoff: %x, filesize: %x, nsects: %d, flags: %x.\n",
...
#if DEBUG
  verbose("symoff: %x, nsyms: %x, stroff: %x, strsize: %x\n",
