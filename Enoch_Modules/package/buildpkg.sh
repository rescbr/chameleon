...
# -
        if [[ "${CONFIG_NVIDIAGRAPHICSENABLER_MODULE}" == 'y' && -f "${SYMROOT}/i386/modules/NVIDIAGraphicsEnabler.dylib" ]]; then
        {
            # Start build NvidiaGraphicsEnabler package module
            choiceId="NVIDIAGraphicsEnabler"
            moduleFile="NVIDIAGraphicsEnabler.dylib"
            mkdir -p "${PKG_BUILD_DIR}/${choiceId}/Root"
            ditto --noextattr --noqtn "${SYMROOT}/i386/modules/$moduleFile" "${PKG_BUILD_DIR}/${choiceId}/Root"
            addTemplateScripts --pkg-rootdir="${PKG_BUILD_DIR}/${choiceId}" \
                               --subst="moduleName=$choiceId"               \
...
# -
        if [[ "${CONFIG_AMDGRAPHICSENABLER_MODULE}" == 'y' && -f "${SYMROOT}/i386/modules/AMDGraphicsEnabler.dylib" ]]; then
        {
...
# -
        if [[ "${CONFIG_INTELGRAPHICSENABLER_MODULE}" == 'y' && -f "${SYMROOT}/i386/modules/IntelGraphicsEnabler.dylib" ]]; then
        {
