...
uint32_t onboard_number		= 1; // TODO: increment the onboard-x(+1) value if the duallink option is true for nVidia and ATi (gfx and HDAU)
...
DevPropDevice *devprop_add_device(DevPropString *string, char *path)
{
	...
	device = malloc(sizeof(DevPropDevice));
	if (!device)
	{
		return NULL;
	}
	...
	if(!string->entries)
	{
		if((string->entries = (DevPropDevice**)malloc(sizeof(device)*DEV_PROP_DEVICE_MAX_ENTRIES))== NULL)
		{
...
/*static int hexdigit(int c)
{
	...
	d = (unsigned)(c - 'A');
	if (d < 6) return d+10;
	printf("[ERROR] hexdigit input error\n");
	return -1;
}*/
...
int hex2dec(unsigned const char *hex) { //long... unsigned const char
	int ret = 0;
	while (*hex && ret >= 0) {
...
/***********************************************************
 * bin2hex creates hexadecimal presentation of binary data *
 ***********************************************************/
char *bin2hex(const uint8_t* bin, char* hex, int len)
{
	int i;
