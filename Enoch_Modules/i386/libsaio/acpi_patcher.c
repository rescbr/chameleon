...
int search_and_get_acpi_fd(const char * filename, const char ** outDirspec)
{
	...
	if (fd < 0)
	{	
		sprintf(dirSpec, "/Extra/%s", filename); // TODO: move default folder to /Extra/Acpi/
		fd = open(dirSpec, 0);
		if (fd < 0)
		{
			sprintf(dirSpec, "bt(0,0)/Extra/%s", filename); // TODO: move default folder to /Extra/Acpi/
			fd = open(dirSpec, 0);
		}
	}
		...
		switch (Platform.CPU.Family) {
			case 0x06:
			{
				switch (Platform.CPU.Model) 
				{
					...
					{
					if ((Platform.CPU.Model == CPU_MODEL_SANDYBRIDGE) || (Platform.CPU.Model == CPU_MODEL_IVYBRIDGE) || (Platform.CPU.Model == CPU_MODEL_HASWELL) || (Platform.CPU.Model == CPU_MODEL_IVYBRIDGE_E5) ||(Platform.CPU.Model == CPU_MODEL_JAKETOWN))
					{
...
int setupAcpi(void)
{
	...
	//ACPI Tables
	bool drop_dmar = false, drop_hpet = false, drop_slic = false, drop_sbst = false, drop_ecdt = false, drop_asft = false;

	getBoolForKey(kDropSSDT, &drop_ssdt, &bootInfo->chameleonConfig);
	getBoolForKey(kDropDMAR, &drop_dmar, &bootInfo->chameleonConfig);
	getBoolForKey(kDropHPET, &drop_hpet, &bootInfo->chameleonConfig);
	getBoolForKey(kDropSLIC, &drop_slic, &bootInfo->chameleonConfig);
	getBoolForKey(kDropSBST, &drop_sbst, &bootInfo->chameleonConfig);
	getBoolForKey(kDropECDT, &drop_ecdt, &bootInfo->chameleonConfig);
	getBoolForKey(kDropASFT, &drop_asft, &bootInfo->chameleonConfig);
