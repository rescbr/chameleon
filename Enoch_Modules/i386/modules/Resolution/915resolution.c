	...
	//	if(map)
		{
            ...
            int i=0;
/*          verbose("unlocked, patching starting at: %X \n", bytes );
            for (i=0; i< 0xfffe; i++){
                char origbytes  []  = {0x00, 0x05};
                if ( bytes[i] == origbytes[0] && bytes[i+1] == origbytes[1])
                {
                    verbose("Addr: %X Before patch: %x %x - ", bytes + i, bytes[i], bytes[i+1]);

                    bytes[i] = 0xA0;
                    bytes[i+1] = 0x05;
                    verbose("after patch: %x %x \n", bytes[i], bytes[i+1]);

                }
            }
            for (i=0; i< 0xfffe; i++){
                char origbytes  []  = {0x20, 0x03};
                if ( bytes[i] == origbytes[0] && bytes[i+1] == origbytes[1])
                {
                    verbose("Addr: %X Before patch: %x %x - ", bytes + i, bytes[i], bytes[i+1]);
...
/*		case 0x00408086: // Core Processor DRAM Controller
		...
		case 0x00698086: // Core Processor DRAM Controller
			verbose(" core proc 1st gen identified\n");
			type = CT_CORE_PROC_1;
			break;
*/			
		...
		default:
			if((id & 0x0000FFFF) == 0x00008086) // Intel chipset
			{
				...
				// Core procs 1st gen: Bus (ff or 7f or 3f), Device 0, Func 1 contains
				// System address decoder (SAD) with devId:
				//  2c01 - Core i7-900 and Extreme Edition, Desktop
				//  2c81 - CoreTM i7-800 and i5-700 Desktop
				//  2c81 - CoreTM i7-900 Mobile Processor Extreme Edition Series, Intel Core i7-800 and i7-700 Mobile
				//  2d01 - Core-i5-600, i3-500 desktop, Pentium 6000, Desktop
				//  2d81 - Core i7-900 and Extreme Edition, Desktop, 32 nm
				//  2cc1 - Xeon Processor C5500/C3500 Series
				// SAD regs 40h-46h are PAM regs
				//
				UInt8  buses[] = {0xff, 0x7f, 0x3f};
				UInt8  bus;
				UInt32 i;
				UInt16 devId;
				...
				for (i = 0; i < sizeof(buses)/sizeof(buses[0]); i++)
				{
					...
					//verbose(" reading %x", (1 << 31 | buses[i] << 16 | 0 << 11 | 1 << 8 | 0));
					outl(CONFIG_MECH_ONE_ADDR, (1 << 31 | buses[i] << 16 | 0 << 11 | 1 << 8 | 0));
					devId = inw(CONFIG_MECH_ONE_DATA + 2);
					//verbose(" = %x\n", devId);
					if (devId == 0x2c01 || devId == 0x2c81 ||
						devId == 0x2d01 || devId == 0x2d81 || devId == 0x2cc1)
					{
						verbose(" core proc identified\n");
						memDevBus = buses[i];
						type = CT_CORE_PROC_1;
				...
				//
				outl(CONFIG_MECH_ONE_ADDR, (1 << 31 | 0 << 16 | 5 << 11 | 0 << 8 | 0));
				devId = inw(CONFIG_MECH_ONE_DATA + 2);
				if (devId == 0x3c28)
				{
					outl(CONFIG_MECH_ONE_ADDR, (1 << 31 | 0 << 16 | 5 << 11 | 0 << 8 | 0x108));
					bus = inb(CONFIG_MECH_ONE_DATA + 1); // "Uncore bus"

					outl(CONFIG_MECH_ONE_ADDR, (1 << 31 | bus << 16 | 12 << 11 | 6 << 8 | 0));
					devId = inw(CONFIG_MECH_ONE_DATA + 2);
					if (devId == 0x3cf4) {
						verbose(" core proc for the LGA-2011 identified\n");
						memDevBus = bus;
						type = CT_CORE_PROC_2011;
						break; // from switch
					}
				}

				verbose(" unknown Intel chipset/proc\n");
...
char detect_bios_type(vbios_map * map, char modeline, int entry_size)
{
	...
	{
//		msglog(" 0x%X", map->mode_table[i].resolution );
...
vbios_map * open_vbios(chipset_type forced_chipset)
{
	...
	if ((map->bios != BT_ATI_1) && (map->bios != BT_ATI_2) && (map->bios != BT_NVDA)) 
	{
		...
		verbose("Other");
		msglog("Started mode table scan at: 0x%X\n", p );
		...
		msglog("Ended mode table scan at: 0x%X\n", p );
		if (map->mode_table == 0)
		{
			close_vbios(map);
			return 0;
...
void unlock_vbios(vbios_map * map)
{
	...
	switch (map->chipset) {
		...
		case CT_3150:
			...
			outb(CONFIG_MECH_ONE_DATA + 2, 0x33);
			break;
		case CT_CORE_PROC_1:	    // Core procs - PAM regs are Bus=memDevBus, Dev=0, Func=1, 40h - 46h
			outl(CONFIG_MECH_ONE_ADDR, (1 << 31 | memDevBus << 16 | 0 << 11 | 1 << 8 | 0x40));
			map->b1 = inb(CONFIG_MECH_ONE_DATA + 1);
			map->b2 = inb(CONFIG_MECH_ONE_DATA + 2);
			outl(CONFIG_MECH_ONE_ADDR, (1 << 31 | memDevBus << 16 | 0 << 11 | 1 << 8 | 0x40));
			outb(CONFIG_MECH_ONE_DATA + 1, 0x33);
			outb(CONFIG_MECH_ONE_DATA + 2, 0x33);
			break;
		case CT_CORE_PROC_2011:	    // Core procs LGA-2011 - PAM regs are Bus=memDevBus, Dev=12, Func=6, 40h - 46h
			outl(CONFIG_MECH_ONE_ADDR, (1 << 31 | memDevBus << 16 | 12 << 11 | 6 << 8 | 0x40));
			map->b1 = inb(CONFIG_MECH_ONE_DATA + 1);
			map->b2 = inb(CONFIG_MECH_ONE_DATA + 2);
			outl(CONFIG_MECH_ONE_ADDR, (1 << 31 | memDevBus << 16 | 12 << 11 | 6 << 8 | 0x40));
			outb(CONFIG_MECH_ONE_DATA + 1, 0x33);
			outb(CONFIG_MECH_ONE_DATA + 2, 0x33);
			break;
		case CT_CORE_PROC_2_3:	    // Core procs - PAM regs are 80h - 86h
	...
	// test if vbios is unlocked
	UInt8 *ptr8 = (UInt8*)(VBIOS_START + 0xa123);
	UInt8 val8 = *ptr8;
	*ptr8 = *ptr8 + 1;
	if (val8 != *ptr8) {
		*ptr8 = val8;
		map->unlocked = TRUE;
		verbose(" vbios unlocked\n");
	}
#if DEBUG
	{
...
void relock_vbios(vbios_map * map)
{
	...
	switch (map->chipset)
	{
		...
		case CT_CORE_PROC_1:
			...
			outb(CONFIG_MECH_ONE_DATA + 2, map->b2);
			break;
		case CT_CORE_PROC_2011:
			outl(CONFIG_MECH_ONE_ADDR, (1 << 31 | memDevBus << 16 | 12 << 11 | 6 << 8 | 0x40));
			outb(CONFIG_MECH_ONE_DATA + 1, map->b1);
			outb(CONFIG_MECH_ONE_DATA + 2, map->b2);
			break;
		case CT_CORE_PROC_2_3:
