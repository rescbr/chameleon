...
long FileLoadDrivers( char * dirSpec, long plugin )
{
	...
	while (1)
	{
		...
		if (!plugin)
		{
			sprintf(gDriverSpec, "%s/%s/%sPlugIns", dirSpec, gFileName, (bundleType == kCFBundleType2) ? "Contents/" : "");
		}
...
long LoadDriverPList( char * dirSpec, char * name, long bundleType )
{
	...
	if(name)
	{
		sprintf(gFileSpec, "%s/%s/%s", dirSpec, name, (bundleType == kCFBundleType2) ? "Contents/MacOS/" : "");
	}
	else
	{
		sprintf(gFileSpec, "%s/%s", dirSpec, (bundleType == kCFBundleType2) ? "Contents/MacOS/" : "");
	}
	...
	if(name)
	{
		sprintf(gFileSpec, "%s/%s/%sInfo.plist", dirSpec, name, (bundleType == kCFBundleType2) ? "Contents/" : "");
	}
	else
	{
		sprintf(gFileSpec, "%s/%sInfo.plist", dirSpec, (bundleType == kCFBundleType2) ? "Contents/" : "");
	}
...
long DecodeKernel(void *binary, entry_t *rentry, char **raddr, int *rsize)
{
	...
	if (kernel_header->signature == OSSwapBigToHostConstInt32('comp'))
	{
		...
		if (OSSwapBigToHostInt32(kernel_header->adler32) != Adler32(binary, uncompressed_size))
		{
