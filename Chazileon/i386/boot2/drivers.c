...
{
    /* IONameMatch support not implemented Azi:blacklist??? */
    return 0;
}
...
{
	//Azi:style
	bool					  patchKernel = false; //Azi:kernelpatcher
  ...
  //Azi:kernelpatcher
  getBoolForKey(kPatchKernelKey, &patchKernel, &bootInfo->bootConfig);
  if (patchKernel == true)
  {
    patch_kernel(binary);
  }
