...
//Azi:include
//#include "libsaio.h" // included on boot.h, acpi_patcher.h, platform.h...
#include "boot.h"
...
static uint64_t acpi10_p; //Azi: aserebln
...
struct acpi_2_fadt *patch_fadt(struct acpi_2_fadt *fadt, struct acpi_2_dsdt *new_dsdt)
{
	...
	if(new_dsdt)
	{
		...
		//Azi: using only getPciRootUID() to signal user supplied acpi table.
...
int setupAcpiNoMod() //Azi:mozo - not used atm.
{
	...
	//Azi: "Slice: because rsdp_mod is a pointer (32bit in i386) to a structure uint64_t*"
...
int setupAcpi(void)
{
	...
	new_dsdt = loadACPITable("DSDT.aml"); //Azi:dsdt
	...
	for (version=0; version<2; version++) {
		...
		//Azi:mozo - commented out; using instead of "FADT: Using custom DSDT!"
		verbose("ACPI version %d\n", version + 1); // always prints so, just print acpi version.
		if (version)
		{
			// XXX aserebln why uint32 cast if pointer is uint64?
			//Azi: See setupAcpiNoMod.
