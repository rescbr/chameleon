...
{
//	{ "fd", 0x00, kBIOSDevTypeFloppy    },  /* DEV_FD */Azi:reminder
...
static int open_bvr(BVRef bvr, const char *filePath, int flags)
{
	...
	// Load entire file into memory. Unnecessary open() calls must be avoided. Azi:---
...
int open_bvdev(const char *bvd, const char *path, int flags)
{
	...
	//Azi: Ok, "path" is checked first so, i've been saying some bs :P
...
BVRef getBootVolumeRef( const char * path, const char ** outPath )
{
    ...
    if (*cp != LP)  // no left paren found  Azi: meaning it can still have a /
    {
