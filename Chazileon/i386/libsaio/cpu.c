...
/*
 * DFE: Measures the TSC frequency in Hz (64-bit) using the ACPI PM timer	<---Azi:tsc
 */
 ...
 * - fsbFrequency = tscFrequency / multi									<---Azi:fsb
...
void scan_cpu(PlatformInfo_t *p)
{
	...
	if ((p->CPU.Vendor == 0x756E6547 /* Intel */) && ((p->CPU.Family == 0x06) || (p->CPU.Family == 0x0f/*Azi:fsb, my fam*/))) {
		if ((p->CPU.Family == 0x06 && p->CPU.Model >= 0x0c) || (p->CPU.Family == 0x0f && p->CPU.Model >= 0x03/*Azi:fsb, my fam/model*/)) {
			/* Nehalem CPU model */
			...
			else
			{
				...
				{
					/* On these models, maxcoef defines TSC freq Azi:fsb mine */
					maxcoef = (msr >> 40) & 0x1f;
				}
				else
				{
