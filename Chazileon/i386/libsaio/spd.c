...
int getDDRspeedMhz(const char * spd)
{
    if (spd[SPD_MEMORY_TYPE]==SPD_MEMORY_TYPE_SDRAM_DDR3) { 
        switch(spd[12])  { // byte 12
        case 0x0f:
            return 1066; // data rate, not Mhz!
        case 0x0c:
            return 1333;
        case 0x0a:
            return 1600;
...
} //Azi: works fine for my DDR2.

#define SMST(a) ((uint8_t)((spd[a] & 0xf0) >> 4)) // msbit?
#define SLST(a) ((uint8_t)(spd[a] & 0x0f)) // lsbit?
...
const char *getDDRSerial(const char* spd)
{
    ...
    if (spd[SPD_MEMORY_TYPE] == SPD_MEMORY_TYPE_SDRAM_DDR3) // DDR3
    {
	sprintf(asciiSerial, "%X%X%X%X%X%X%X%X", SMST(122) /*& 0x7 Azi:msb*/, SLST(122), SMST(123), SLST(123),
											SMST(124), SLST(124), SMST(125), SLST(125));

    }
    else if (spd[SPD_MEMORY_TYPE] == SPD_MEMORY_TYPE_SDRAM_DDR2) // DDR2 or DDR
    { 
	sprintf(asciiSerial, "%X%X%X%X%X%X%X%X", SMST(95) /*& 0x7*/, SLST(95), SMST(96), SLST(96),
											SMST(97), SLST(97), SMST(98), SLST(98));
    }
...
const char * getDDRPartNum(char* spd, uint32_t base, int slot)
{
	...
	memset(asciiPartNo, 0, sizeof(asciiPartNo));
	...
	for (i = start; i < start + sizeof(asciiPartNo); i++)
	{
		...
		if (isspace(c)) //Azi: break at first space
			break;
	}
